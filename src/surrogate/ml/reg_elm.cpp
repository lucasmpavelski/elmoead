...
void RegELMTrainer::updateWeights(const_matrix_ref mX, const_matrix_ref mY,
                                  const SLFNPredictor& pred, SLFNNetwork& net)
{
    const size_t nsmp = mX.cols();
    const size_t nh = net.noHidden();

    net.resetWeights();

    static const bool bias = 0;
    H.resize(nh, nsmp + bias);
    for (int i = 0; i < nsmp; ++i)
        pred.activationFunction(net, mX.col(i), H.col(i));

    if (bias)
        H.col(nsmp) = SLFN::vector::Ones(nh);
    ...
    if (nh <= nsmp)
    {
        solver.compute(H * H.transpose() + SLFN::matrix::Identity(nh, nh) * inv_C);
        net.outputWeights() = solver.solve(H * mY);
    }
    else
    {
        solver.compute(H.transpose() * H + SLFN::matrix::Identity(nsmp + bias,
                                                                  nsmp + bias) * inv_C);
        net.outputWeights() = H * solver.solve(mY);
    }
}
